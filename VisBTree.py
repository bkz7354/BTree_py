...
class VisBtree:
    class Node:
        def __init__(self, t, is_leaf, tree_instance, manager):
            self.t = t
            self.fill = 0
            self.key = [None]*(2*t-1)
            self.c = [None]*(2*t)
        ...
        def insert(self, val):
            ...
            if self.is_leaf:
                ...
                if self.c[pos].fill == 2*self.t-1:
                    ...
                    self.manager.split_child(self, pos)
                    if val > self.key[pos]:
                        pos += 1
        ...
        def split_child(self, c_id):
            l = self.c[c_id]
            r = self.inst.Node(self.t, l.is_leaf, self.inst, self.manager)
    ...
    def insert(self, val):
        if self.root == None:
            self.root = self.Node(self.t, True, self, self.manager)
            self.root.key[0] = val
            self.root.fill = 1

            self.manager.create_root(self.root, val)
            return
        
        if self.root.fill == 2*self.t-1:
            new_root = self.Node(self.t, False, self, self.manager)
